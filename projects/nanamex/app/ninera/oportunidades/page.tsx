...
import { OpportunityFilters } from "@/components/ninera/opportunity-filters";
import { matchesAvailabilityWindow, normalizeOpportunityFilters, scoreNecesidadForNinera, toMatchNinera, type StoredNecesidadForMatching, type OpportunityCardData } from "@/lib/ninera/opportunities";
...
export default async function ExploreOpportunitiesPage({ searchParams }: { searchParams?: SearchParams } = {}) {
  ...
  const filters = normalizeOpportunityFilters(params);
  const opportunities = (Array.isArray(needsResult.data) ? needsResult.data : [])
    .filter((row) => (row as { estado?: string }).estado === "activa")
    .filter((row) => matchesAvailabilityWindow(row as unknown as StoredNecesidadForMatching, ninera, filters.day, filters.start, filters.end))
    ...
    .filter((item) => !filters.zona || item.zona === filters.zona).filter((item) => !filters.modalidad || item.modalidad === filters.modalidad)
    .filter((item) => !filters.minPay || item.pagoMax >= filters.minPay).filter((item) => !filters.maxPay || item.pagoMin <= filters.maxPay)
    .sort((a, b) => b.score - a.score || b.recency.localeCompare(a.recency) || a.id.localeCompare(b.id));
  return <><NineraNavigation /><main className="mx-auto min-h-screen w-full max-w-[1120px] bg-bg px-4 py-8 pb-24 text-ink-900 sm:px-6 lg:ml-[248px] lg:w-[calc(100%-248px)] lg:px-10 lg:py-12"><header><h1 className="text-h1">Explorar vacantes</h1><p className="mt-2 text-body text-ink-600">Encuentra oportunidades abiertas que podrían coincidir contigo.</p></header><div className="mt-6 flex flex-col gap-6 lg:flex-row"><OpportunityFilters {...filters} /><section className="min-w-0 flex-1"><div className="mb-4 flex items-center justify-between gap-3"><p className="text-body-sm text-ink-600">{opportunities.length} vacantes disponibles</p><Link href="/ninera/oportunidades/recibidas" className="inline-flex min-h-11 items-center text-button text-primary-700 underline">Oportunidades recibidas</Link></div>{opportunities.length === 0 ? <div className="border-t border-border pt-6"><h2 className="text-headline">No encontramos vacantes con estos filtros</h2><p className="mt-2 text-body text-ink-600">Las vacantes abiertas aparecerán aquí</p><p className="mt-2 text-body text-ink-600">Intenta ampliar tu zona, modalidad, rango de pago o disponibilidad.</p><Link href="/ninera/oportunidades" className="mt-3 inline-flex min-h-11 items-center text-button text-primary-700 underline">Limpiar filtros</Link></div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{opportunities.map((item) => <OpportunityCard key={item.id} opportunity={item} />)}</div>}</section></div></main></>;
}
