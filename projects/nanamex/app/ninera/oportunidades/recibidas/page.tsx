...
function card(row: Row): OpportunityCardData | null {
  ...
  return { id: row.id, zona: zona ?? "", modalidad: need.modalidad, pagoMin: need.pago_min, pagoMax: need.pago_max, fechaInicio: need.fecha_inicio, score: row.match_score_snapshot, factors: row.match_checklist_snapshot ?? {}, pushed: true, recency: need.updated_at };
}
...
export default async function ReceivedOpportunitiesPage() {
  ...
  return <><NineraNavigation /><main className="mx-auto min-h-screen w-full max-w-[1120px] bg-bg px-4 py-8 pb-24 text-ink-900 sm:px-6 lg:ml-[248px] lg:w-[calc(100%-248px)] lg:px-10 lg:py-12"><header><h1 className="text-h1">Oportunidades recibidas</h1><p className="mt-2 text-body text-ink-600">Vacantes que coinciden con lo que buscas.</p></header>{opportunities.length === 0 ? <section className="mt-8 border-t border-border pt-8"><p className="text-body text-ink-600">Aún no tienes oportunidades. Mientras tanto, explora vacantes abiertas.</p><Link href="/ninera/oportunidades" className="mt-3 inline-flex min-h-11 items-center text-button text-primary-700 underline">Explorar vacantes</Link></section> : <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{opportunities.map((item) => <OpportunityCard key={item.id} opportunity={item} />)}</div>}</main></>;
}
