...
| Revisited success URL after an abandoned checkout | A local `pendiente` boundary older than 30 minutes is checked against Stripe before cleanup. An old open session is expired first; only a confirmed `expired`/never-created boundary is marked `fallido` and its URL cleared. Recent open/complete returns are preserved, while provider errors fail closed. No URL/query parameter grants access, and stale returns route to a new paywall attempt. |
