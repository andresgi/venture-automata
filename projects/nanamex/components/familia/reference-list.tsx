...
export function ReferenceList({ references, rowsOnly = false }: { references: Reference[]; rowsOnly?: boolean }) {
    ...
    <ul className="mt-4 divide-y divide-border border-y border-border">
      {references.map((reference) => (
        <li key={reference.id} className="py-4 text-body text-ink-900">
          ...
          <p className="text-body-sm text-ink-600">{reference.relacion}, {reference.periodo}</p>
          {reference.contacto && <p className="mt-1 text-body-sm text-ink-600">Contacto: {reference.contacto}</p>}
        </li>
      ))}
    </ul>
  );
